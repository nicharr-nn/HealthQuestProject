      ...
      <div v-else class="grid gap-4">
        ...
        >
          ...
          <div
            ...
            class="mb-4 p-3 bg-white border-l-4 border-blue-500 rounded-lg"
          >
          ...
          <div class="flex flex-wrap gap-2 pt-4 border-t border-gray-200">
            <template v-if="request.status === 'pending'">
              <button
                class="px-3 py-1.5 text-xs font-semibold rounded bg-green-500 text-white hover:bg-green-600 transition"
                @click="updateRequestStatus(request.relationship_id, 'accepted')"
              >
                ✓ Accept
              </button>
              <button
                class="px-3 py-1.5 text-xs font-semibold rounded bg-red-500 text-white hover:bg-red-600 transition"
                @click="updateRequestStatus(request.relationship_id, 'rejected')"
              >
                ✕ Reject
              </button>
              <button
          ...
          >
            <div class="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
              ...
              <!-- Loading / Error -->
              <div v-if="modalLoading" class="text-center py-16 text-gray-500">
                Loading profile...
              </div>
              <div v-else-if="modalError" class="text-center py-16 text-red-500">
                {{ modalError }}
              </div>
...
async function viewDetails(request) {
  ...
  try {
    const res = await fetch(`${API_URL}/api/member/profile/${request.memberId}/`, {
      credentials: 'include',
    })
