<template>
  <div
    class="min-h-screen bg-gradient-to-br from-gray-100 via-yellow-50 to-gray-100 p-4 sm:p-6 lg:p-8"
  >
    <!-- Header with Stats -->
    <div class="max-w-7xl mx-auto mb-6 sm:mb-8">
      ...
      <div class="font-body flex flex-wrap gap-4 sm:gap-6 lg:gap-8 items-center">
        ...
        <div class="flex items-center gap-2">
          ...
          <div
            class="bg-gray-300 text-gray-700 px-3 sm:px-4 py-1 rounded-full text-sm font-semibold"
          >
            {{ progressPercentage }}%
          </div>
        </div>
        ...
        <div class="ml-auto flex gap-6 sm:gap-8 lg:gap-12">
          ...
          <div class="text-right">
            <div class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-800">
              {{ nextRequirement || 'MAX' }}
            </div>
    ...
    <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6">
      ...
      <div class="lg:col-span-3 space-y-4 sm:space-y-6">
        <!-- Profile Card -->
        <div
          class="font-body bg-gradient-to-br from-[#88ACEA] to-[#88ACEA] rounded-3xl p-6 sm:p-8 text-white shadow-xl overflow-hidden relative h-full min-h-[400px] flex flex-col justify-center"
        >
          <div class="relative z-10">
            <div
              class="w-24 h-24 sm:w-32 sm:h-32 rounded-full overflow-hidden mx-auto mb-4 sm:mb-6 border-4 border-white/30 shadow-lg"
            >
              <img
            ...
            <div
              v-if="memberProfile && memberProfile.member_id"
              class="bg-white/20 backdrop-blur-sm rounded-2xl px-4 py-3 text-center"
            >
              <div class="text-sm text-white/80 mb-1">Member ID</div>
              <div class="text-lg sm:text-xl font-bold tracking-wider">
          ...
          ></div>
          <div
            class="absolute -bottom-10 -left-10 w-32 h-32 sm:w-40 sm:h-40 bg-white/10 rounded-full"
          ></div>
        </div>
      </div>
      ...
      <div class="lg:col-span-6 space-y-4 sm:space-y-6">
        ...
        <div class="bg-white rounded-3xl p-6 sm:p-8 shadow-lg">
          <div
            class="font-body flex flex-col sm:flex-row items-start justify-between mb-4 sm:mb-6 gap-2"
          >
            <div>
          ...
          <div class="mb-4 sm:mb-6">
            <div class="flex items-center justify-between mb-2">
              <span class="font-body text-xs sm:text-sm font-medium text-gray-600"
                >Progress to next level</span
              >
              <span class="font-body text-xs sm:text-sm text-gray-800"
                >{{ xp }} / {{ nextRequirement || 'MAX' }} XP</span
              >
            </div>
          ...
          <div class="mt-6 sm:mt-8">
            <h4 class="font-body text-xs sm:text-sm font-semibold text-gray-700 mb-3 sm:mb-4">
              Weekly Activity
            </h4>
        ...
        <div class="bg-white rounded-3xl p-6 sm:p-8 shadow-lg">
          <div
            v-if="store.profile?.role === 'member'"
            class="flex flex-col sm:flex-row items-start justify-between mb-4 gap-2"
          >
      ...
      <div class="lg:col-span-3 space-y-4 sm:space-y-6">
        ...
        <div class="bg-white rounded-3xl p-5 sm:p-6 shadow-lg">
          ...
          <div class="space-y-3 sm:space-y-4">
            <div
              class="text-center p-3 sm:p-4 bg-gradient-to-br from-[#F3C6C1] to-[#F3C6C1] rounded-2xl"
            >
              <div class="text-2xl sm:text-3xl font-extrabold text-[#9C6963] mb-1">
                {{ analytics.weeklyImprovement }}%
              </div>
              <div class="text-xs text-gray-700 font-medium">Weekly Improvement</div>
            </div>
            <div
              class="text-center p-3 sm:p-4 bg-gradient-to-br from-[#a8d5e2] to-[#a8d5e2] rounded-2xl"
            >
              <div class="text-2xl sm:text-3xl font-extrabold text-[#05668d] mb-1">
        ...
        <!-- Recipe Library Card -->
        <div
          v-if="currentLevel !== 'Bronze'"
          class="bg-gradient-to-br from-[#f4e285] to-[#f4e285] rounded-3xl p-5 sm:p-6 shadow-lg"
        >
          <div class="flex items-start justify-between mb-3 sm:mb-4">
            <h3 class="font-subtitle text-base sm:text-lg text-gray-800">Recipe Library</h3>
            <div
              class="font-body bg-emerald-500 text-white px-2.5 py-1 rounded-lg text-[10px] font-extrabold"
            >
              NEW
            </div>
          </div>

          <p class="text-xs sm:text-sm text-gray-700 mb-4 sm:mb-6">
            Access 10+ healthy recipes and meal plans
          </p>
...
export default {
  ...
  setup() {
    ...
    async function loadMemberProfile() {
      ...
      try {
        const res = await fetch(`${API_URL}/api/member/member-profile/`, {
          credentials: 'include',
        })
        if (!res.ok) {
          if (res.status === 404) {
    ...
    async function loadAnalytics() {
      ...
      try {
        const res = await fetch(`${API_URL}/api/workout/analytics/`, {
          credentials: 'include',
        })
        if (!res.ok) {
          const err = await res.json().catch(() => ({}))
    ...
    async function loadWeeklyActivity() {
      try {
        const res = await fetch(`${API_URL}/api/workout/analytics/weekly-activity/`, {
          credentials: 'include',
        })
        if (!res.ok) {
          ...
          weekDays.value = data.map((item) => ({
            ...
            height:
              typeof item.height === 'number'
                ? item.height
                : Math.max(20, Math.min(100, (item.count || 0) * 10)),
