...
class WorkoutProgramAccessTests(TestCase):
    ...
    def test_normal_user_sees_only_public_programs(self):
        ...
        member_profile = member.userprofile

        coach_user1 = User.objects.create_user(username="coach1", password="pass")
        coach1 = Coach.objects.create(user=coach_user1.userprofile, status_approval="approved")

        coach_user2 = User.objects.create_user(username="coach2", password="pass")
        coach2 = Coach.objects.create(user=coach_user2.userprofile, status_approval="approved")
        ...
        WP.objects.create(
            ...
            **({"coach": coach1.user} if "coach" in [f.name for f in WP._meta.get_fields()] else {})
        )
        ...
        WP.objects.create(
            ...
            category="general",
            is_public=False,
            **({"coach": coach2.user} if "coach" in [f.name for f in WP._meta.get_fields()] else {})
        )
    ...
    def test_coach_sees_private_programs_for_their_own_account(self):
        ...
        """
        coach_user = User.objects.create_user(username="coachx", password="pass")
        # mark profile as coach so view's role check passes
        coach_user.userprofile.role = "coach"
        coach_user.userprofile.save()
        coach = Coach.objects.create(user=coach_user.userprofile, status_approval="approved")
        ...
        WP.objects.create(
            ...
            category="general",
            is_public=False,
           **({"coach": coach.user} if "coach" in [f.name for f in WP._meta.get_fields()] else {})
        )
